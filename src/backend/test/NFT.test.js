...
describe("NFT Functions", function(){
    ...
    beforeEach(async function(){
        ...
        nft = await NFT.deploy("Inspiro NFT", "INSP", "exampleurl/", 100, toWei(1));
    })

    //Passed
    // it("Should mint tokens at 1eth, check values", async function(){
    //     await nft.connect(deployer).mint(1, {value: toWei(1)})
    ...
    //     expect(await ethers.provider.getBalance(nft.address)).to.equal(toWei(3))
    // })
    it("Get functions should all be working", async function(){
        expect(await nft.tokenCount()).to.equal(0)
        expect(await nft.mintPrice()).to.equal(toWei(1))
    })
})